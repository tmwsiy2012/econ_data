...
FROM onet_20_2.occupation_data ocd
	INNER JOIN bls_oe.occupation occ ON REPLACE(LEFT(ocd.onetsoc_code, 7), '-','')=occ.occupation_code;


SELECT *
  FROM onet_20_2.occupation_data ocd
	INNER JOIN onet_20_2.knowledge know ON know.onetsoc_code=ocd.onetsoc_code
	INNER JOIN onet_20_2.content_model_reference cmr ON cmr.element_id=know.element_id;
------------------------------------------------------------------------------------------------
drop table if exists `onet_20_2`.`tmp_summary`;
CREATE TABLE `onet_20_2`.`tmp_summary` (
...
ALTER TABLE `onet_20_2`.`tmp_summary`
...
SELECT ocd.onetsoc_code, ocd.title, cmr.element_id, cmr.element_name, skill.scale_id, skill.data_value, cmr.description
  FROM onet_20_2.occupation_data ocd
	INNER JOIN onet_20_2.skills skill ON skill.onetsoc_code=ocd.onetsoc_code
	INNER JOIN onet_20_2.content_model_reference cmr ON cmr.element_id=skill.element_id
WHERE
  skill.scale_id="IM";
